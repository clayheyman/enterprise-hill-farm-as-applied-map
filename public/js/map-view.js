(function () {
  ...
  function render() {
    ...
    document.getElementById('totalAmount').innerHTML = `${o.totalAmount} ${unitLabel(o.unit)}`;
    document.getElementById('totalRate').textContent = `${o.avgRatePerAcre} ${unitLabel(o.unit)}/ac`;
  ...
  function initMap() {
    ...
    job.fields.forEach((f) => {
      ...
      layer.bindPopup(`<strong>${escapeHtml(f.displayName)}</strong><br>${f.totalAcres} ac · ${f.totalAmount} ${unitLabel(f.unit)} · ${f.avgRatePerAcre} ${unitLabel(f.unit)}/ac`);
      layer.addTo(group);
    });
    group.addTo(map);
