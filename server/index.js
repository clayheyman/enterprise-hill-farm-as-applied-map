...
router.post('/api/admin/upload', async (req, res) => {
  ...
  // Try to auto-match saved boundaries from the field library by plotId, then by fieldName.
...
router.post('/api/admin/jobs/:id/publish', async (req, res) => {
  ...
  // Save any newly-drawn boundaries into the reusable field library, keyed by plot id / field name.
  const library = store.listFieldLibrary();
  job.fields.forEach((f) => {
    if (!f.boundary) return;
    const key = f.plotId || f.displayName;
    let entry = library.find((l) => (f.plotId && l.plotId === f.plotId) || l.name === f.displayName);
    if (!entry) {
      entry = { id: generateId(), plotId: f.plotId || null, name: f.displayName, color: f.color, boundary: f.boundary };
    } else {
