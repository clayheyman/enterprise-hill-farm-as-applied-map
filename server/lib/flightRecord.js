...
function summarizeFlightRecords(records) {
  ...
  // Group into fields. A physical field is the same field across multiple
  // battery passes even when Talos doesn't give us a Plot ID for every row,
  // so we fall back through the strongest signal available rather than
  ...
  const fields = groupOrder.map((key, i) => {
    ...
    const plotId = key.startsWith('plot:') ? key.slice(5) : null;
